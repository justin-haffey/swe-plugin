...
title: "Code Mode MCP Tool Interface Improvement Pattern - Awesome Agentic Patterns"
---

[Skip to content](https://agentic-patterns.com/patterns/code-first-tool-interface-pattern/#problem)

# Code Mode MCP Tool Interface Improvement Pattern NEW
...
Right now, Code Mode especially fails at cases where intelligence needs to be _inserted in the middle of code_. Example: A spreadsheet with 100 emails where you want to write a _personalized_ email for each entry. The `body` argument for that `send_email` call must be computed using LLM for personalization.
...
AWS APIsMCP Server(AWS Tools)Secure BindingsV8 Isolate(Ephemeral)LLMUserAWS APIsMCP Server(AWS Tools)Secure BindingsV8 Isolate(Ephemeral)LLMUserCode Generation & Execution Phase// Generated code orchestrates full workflowconst vpc = await createVPC({name: "demo-vpc"})const igw = await createInternetGateway(vpc.id)const sg = await createSecurityGroup(vpc.id, sshRules)const instance = await launchEC2Instance({ type: "m4.large", vpcId: vpc.id, sgId: sg.id})await tagResources(\[vpc, igw, sg, instance\])Security Group creationEC2 instance launchResource tagging💀 Isolate destroyedWorkflow state cleared🎯 Single workflow execution6 AWS API calls → 1 condensed result"Provision EC2 m4 instance with SSH access"Generate TypeScript workflow:createVPC() → createIGW() → createSG() → launchEC2()Execute createVPC()Create VPC requestvpc\_create API call (with creds)VPC createdVPC detailsReturn vpc objectExecute createInternetGateway()Create IGW + attachigw\_create + attach API callsIGW attachedIGW detailsReturn igw objectExecute remaining workflowAll resources provisioned{vpcId, instanceId, publicIP, sshCommand}"Infrastructure ready! SSH: ssh -i key.pem ec2-user@54.x.x.x"
...
5. LLM generates TypeScript code using the provided APIs
6. Code runs in isolated V8 environment
7. Bindings provide controlled access to tools
8. Results return to the agent for further processing
