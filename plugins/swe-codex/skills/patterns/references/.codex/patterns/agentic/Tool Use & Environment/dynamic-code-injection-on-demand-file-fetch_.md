...
title: "Dynamic Code Injection (On-Demand File Fetch) - Awesome Agentic Patterns"
---

[Skip to content](https://agentic-patterns.com/patterns/dynamic-code-injection-on-demand-file-fetch/#problem)
...
- Build a **listener** in your chat frontend or CLI that recognizes `@` and `/load` tokens.
- Map recognized tokens to file paths; verify permissions if outside project root.
- Read file text, run a **line-range parser** or **AST-based snippet extractor** if needed.
- Replace the token in the outgoing prompt with `/// BEGIN <filename> …content… /// END <filename>`.
- Forward the augmented prompt to the LLM for inference.
