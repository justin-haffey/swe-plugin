...
- If already running as `repo-author`, or when the allocation contains `BUILT_IN_WORKER_FALLBACK`, perform documentation work directly instead of delegating another wrap-up. Required check execution still routes through `$swe-test` to the tester.
...
1. Unless running as `repo-author` or under `BUILT_IN_WORKER_FALLBACK`, attempt to delegate to `repo-author`. If that role cannot be resolved or started, delegate once to the built-in `worker` and include `Execution mode: BUILT_IN_WORKER_FALLBACK`; the worker must perform documentation work directly and must not delegate another wrap-up. Give the documentation agent the repository root, completed-goal summary, known changed paths, check receipts already available, this skill name and the required `$swe-test` route.
...
6. Request repository-native checks for the attributable change plus Markdown link/path checks and `git diff --check` through `$swe-test` -> `test-runner` (`gpt-5.6-luna`, `medium`). Reuse current applicable receipts and request only checks affected by final documentation edits or unresolved concerns. The documentation author fixes failures; the tester executes and returns attributable receipts. Missing tester/tool permission or required validation remains blocked.
...
Inspect the live Git diff and relevant change artifacts, reconcile README files, change AGENTS.md only for durable governance changes, follow repository-local documentation rules, request checks through $swe-test -> test-runner (gpt-5.6-luna, medium), and prepare an exact review handoff. Fix documentation yourself; do not delegate another wrap-up or execute checks outside the tester route. Do not stage or commit. Return exact changed paths, receipts, preserved unrelated changes or a precise blocker, then pause for user review. Do not call update_goal.
```
...
- a built-in worker allocation contained `BUILT_IN_WORKER_FALLBACK` and did not delegate another wrap-up;
- README claims are grounded in current artifacts and source;
- every AGENTS edit represents a real durable governance change;
- required repository-native checks and `git diff --check` have current attributable tester receipts with truthful results;
