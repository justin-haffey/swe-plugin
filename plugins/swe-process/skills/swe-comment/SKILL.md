...
8. Request the narrowest required formatting, compile, lint, or test checks, including `git diff --check`, through `$swe-test` to `test-runner` (`gpt-5.6-luna`, `medium`). Checks must not rewrite files; the commenter owns documentation repairs. Confirm from the final diff that behavior, executable tokens, dependencies, tests, generated outputs, and non-code documentation did not change. Missing required execution capability remains a reported blocker.
