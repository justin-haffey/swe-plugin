...
Tests run in the actual owning child checkout, including dirty files, against one canonical fixture set. Never copy repositories, source trees, prior results or whole corpora for verification. Follow `$swe-test` capture limits and rerun only affected checks.
