...
Tests and fixture definitions belong in the owning solution's native test tree. Execute against the actual child checkout, including uncommitted files; never create copied source trees or run from portfolio scratch. Keep one read-only fixture set and pass its path/digest to consumers. Mutation tests create only the small case they mutate in child-local scratch. Reproduction uses the same inputs and separate outputs, without duplicating repositories or whole corpora. Do not recursively include prior results, source exports or build directories in retained observations.

The recorder requires result and working directories inside the declared checkout. Use child-local fresh observations; after retaining the receipt and required failure diagnostics, remove only run-owned scratch after checking references and tracked files. Raw historical receipts remain immutable. Retention is separate from correctness: hashes alone do not replace bytes explicitly required by a criterion. Optional `max_capture_bytes` defaults to 25 MiB; justify an increase in `scope_rationale`. Oversized observations/logs are Blocked and remaining checks stop when capture exceeds the budget. This bounds recorder retention between commands, not arbitrary process writes or peak memory; native generators must also bound scratch. Compact JSON keeps the existing receipt schema and full fingerprints intact.
