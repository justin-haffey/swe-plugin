...
3. Verify Goal operations, required SWE/SWA skills, `$orchestrate -complex`, `$swe-test`, effective tester configuration and child-project task transport/result retrieval. `$swe-bridge` must start in the exact child repository's project context; `/fork` is optional only when it can enter that project. A parent-task subagent or changed tool working directory does not qualify. No new user-visible task without user or active host authorization.
